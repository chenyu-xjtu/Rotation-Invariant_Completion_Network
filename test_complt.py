...
def train():
    ...
    val(net, val_loss_meters, dataloader_test)


def val(net, val_loss_meters, dataloader_test):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(dataloader_test)):
            ...
            gt = complete_pointcloud2.transpose(2, 1).float().cuda()
            ...
            if args.save_vis:
                for z in range(args.batch_size):
                    ...
                    if idx in idx_to_plot:
                        ...
                        plot_single_pcd(inputs[z].transpose(0,1).cpu().numpy(), os.path.join(save_partial_path, pic))
