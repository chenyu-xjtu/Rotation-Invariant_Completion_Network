...
def group_index(nsample, radius, xyz, new_xyz, group='knn'):
    if group == 'knn':
        idx = knn_point(nsample, xyz, new_xyz.contiguous()) #xyz(B,3,3) new_xyz(B,2048,3)
    elif group == 'ball':
        idx = ball_query(radius, nsample, xyz, new_xyz.contiguous())
        idx = idx.long()
    else:
...
def sample_and_group(npoint, radius, nsample, xyz, norm):
    ...
    new_xyz, new_norm = sample(npoint, xyz, norm=norm, sampling='fps') #new_xyz(B,2048,2045) new_norm(B,2048,3)
